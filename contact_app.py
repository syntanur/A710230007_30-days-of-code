...
class ContactApp(QMainWindow):
    def __init__(self):
        ...
        query = QSqlQuery()
        query.exec_('CREATE TABLE IF NOT EXISTS contacts (name TEXT, email TEXT)')
    ...
    def update_contact(self):
        ...
        if current_item:
            ...
            query = QSqlQuery()
            query.prepare('UPDATE contacts SET name=?, email=? WHERE name=?')
            query.bindValue(0, name)
            query.bindValue(1, email)
            query.bindValue(2, selected_name)
    ...
    def show_contact(self, item):
        ...
        if query.exec_() and query.next():
            ...
            self.email_input.setText(query.value(1))
